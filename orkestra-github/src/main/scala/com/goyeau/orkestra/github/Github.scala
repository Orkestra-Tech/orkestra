...
import com.goyeau.orkestra.{BuildInfo, OrkestraConfig}
import com.goyeau.orkestra.filesystem.{Directory, LocalFile}
...
trait Github extends LazyLogging {
  protected val orkestraConfig: OrkestraConfig
      ...
      response <- Http().singleRequest(
        HttpRequest(
          ...
          HttpEntity(
            CheckStatus(
              state,
              s"${githubConfig.uri}/#/logs/${orkestraConfig.runInfo.runId.value}",
              state.description,
...
object Github extends Github {
  implicit override val orkestraConfig: OrkestraConfig = OrkestraConfig.fromEnvVars()
